...
To stop the robot from getting pushed around when sticks are released, use `ManualDrive.driveOrHold()`. It automatically holds position when driver input stops.

Replace `follower.manual()` with `ManualDrive.driveOrHold()`:
...
You can also use `driveOrHold()` with field-centric drive powers:
