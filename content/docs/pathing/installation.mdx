---
title: Installation
description: Installing Pedro Pathing
---

import PedroImplementation from "@/app/PedroImplementation";

There are two ways to install Pedro Pathing. The quickstart is for people who don't have code for their robot yet, if you do have code for your robot then use the manual installation.

## Using the Quickstart

The quickstart is the easiest way to install Pedro Pathing.

1. In Android Studio, go to `Main Menu -> File -> New -> Project from
Version Control`. For the URL, enter
`https://github.com/Pedro-Pathing/Quickstart.git`.
2. **OR** run `git clone https://github.com/Pedro-Pathing/Quickstart.git`.
Make sure you have [git](https://git-scm.com/) installed first.

That's it! You have now installed Pedro Pathing.

## Manually

In your `build.dependencies.gradle`, add the following to the `repositories
{ }` block:

```groovy
maven { url = "https://mymaven.bylazar.com/releases" }
```

Then, add the following to the `dependencies { }` block:

<PedroImplementation/>
```groovy
implementation 'com.bylazar:fullpanels:1.0.6'
```

Next, perform a Gradle sync by pressing "sync now" in the blue banner that
has appeared.

Then, navigate to `File > Project Structure > Modules` and change the `Compile Sdk Version` to 34 for `FtcRobotController` and `TeamCode`.
Then press `Apply` and `OK`.

Lastly, run the following commands as stated based on your operating system:

### Linux and macOS:
#### **Run these commands in your main folder with your robot code:**

```groovy
curl -O https://pedropathing.com/manualinstall.sh
sudo chmod +x manualinstall.sh
./manualinstall.sh
```

### Windows:
Copy the files from the [`pedroPathing` package in the quickstart](https://github.com/Pedro-Pathing/Quickstart/tree/master/TeamCode/src/main/java/org/firstinspires/ftc/teamcode/pedroPathing)
into your code. More simplicity for Windows install coming soon.