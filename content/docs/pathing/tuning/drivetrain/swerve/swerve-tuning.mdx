...
To do this, run the [Analog Min / Max Tuner](docs/swerve/AnalogMinMaxTuner.java) opmode. After you press start, slowly rotate each
...
Then, run the [SwerveOffsetsTuner](docs/swerve/SwerveOffsetsTuner.java) opmode (do this off the ground, the servos and motors will spin). This opmode will attempt to drive each pod to its zero position. However, there are a few
...
If you have completed all of the above steps correctly, when running [SwerveOffsetsTuner](docs/swerve/SwerveOffsetsTuner.java), the robot should drive forward slowly when you put it on the ground.
...
then, run the [SwerveTurnTuner](docs/swerve/SwerveTurnTuner.java) opmode. Your pods should turn to be perpendicular to the center of the robot, forming
...
the motors are running the wrong direction, but they are all correct for the [SwerveOffsetsTuner](docs/swerve/SwerveOffsetsTuner.java) to move your robot forward, then you almost certainly
...
## Pod PIDF Tuning
Finally, you need to tune your pod PIDF coefficients. Run the [SwervePIDTuner](docs/swerve/SwervePIDTuner.java) opmode to tune your PIDs. You can do this using whatever method you prefer to tune your PIDFs. What I've found to work
